    ...
    <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-950 text-zinc-100 overflow-hidden font-sans shadow-lg">
      ...
      <div className="flex items-center gap-1 px-3 sm:px-4 pt-2.5 bg-zinc-950 border-b border-zinc-800/80 overflow-x-auto text-xs no-scrollbar">
        <button
          ...
          className={cn(
            "px-3 py-1.5 font-medium rounded-t-lg transition-colors border-b-2 whitespace-nowrap cursor-pointer",
            activeTab === "overview"
        ...
        {activeTab === "overview" && (
          <div className="space-y-3 sm:space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-2.5">
              <div className="p-2.5 sm:p-3 rounded-lg bg-zinc-900/80 border border-zinc-800">
                <div className="text-[10px] sm:text-[11px] font-mono text-zinc-400">Target Uptime</div>
                <div className="mt-1 text-base sm:text-lg font-bold font-mono text-emerald-400">99.98%</div>
                <div className="text-[9px] sm:text-[10px] text-zinc-500">SLA Maintained</div>
              </div>

              <div className="p-2.5 sm:p-3 rounded-lg bg-zinc-900/80 border border-zinc-800">
                <div className="text-[10px] sm:text-[11px] font-mono text-zinc-400">p95 Latency</div>
                <div className="mt-1 text-base sm:text-lg font-bold font-mono text-cyan-400">{currentLatency} ms</div>
                <div className="text-[9px] sm:text-[10px] text-emerald-400">▼ 3ms vs baseline</div>
              </div>

              <div className="p-2.5 sm:p-3 rounded-lg bg-zinc-900/80 border border-zinc-800">
                <div className="text-[10px] sm:text-[11px] font-mono text-zinc-400">Requests / Min</div>
                <div className="mt-1 text-base sm:text-lg font-bold font-mono text-zinc-100">
                  {pingCount.toLocaleString()}
                </div>
                <div className="text-[9px] sm:text-[10px] text-zinc-500">Live SSE stream</div>
              </div>

              <div className="p-2.5 sm:p-3 rounded-lg bg-zinc-900/80 border border-zinc-800">
                <div className="text-[10px] sm:text-[11px] font-mono text-zinc-400">Error Budget</div>
                <div className="mt-1 text-base sm:text-lg font-bold font-mono text-emerald-400">0.02%</div>
                <div className="text-[9px] sm:text-[10px] text-zinc-500">Threshold: &lt; 0.5%</div>
              </div>
            </div>
            ...
            <div className="space-y-2 pt-1">
              ...
              {services.map((svc, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between p-2 sm:p-2.5 rounded-lg bg-zinc-900/40 border border-zinc-800/80 text-xs hover:bg-zinc-900/70 transition-colors"
                >
                  <div className="flex items-center gap-2 sm:gap-2.5 min-w-0">
                    <span className="w-2 h-2 rounded-full bg-emerald-400 shrink-0" />
                    <span className="font-medium text-zinc-200 truncate">{svc.name}</span>
                  </div>
                  <div className="flex items-center gap-2 sm:gap-3 font-mono text-xs shrink-0">
                    <span className="text-zinc-400 text-[11px] sm:text-xs">{svc.latency}</span>
                    <span className="px-1.5 sm:px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-400 text-[10px] sm:text-[11px] border border-emerald-500/20">
                      {svc.status}
                    </span>
                  </div>
                </div>
            ...
            <div className="p-3 rounded-lg bg-zinc-900/90 border border-zinc-800 font-mono text-xs text-zinc-300 space-y-2">
              <div className="flex items-center justify-between text-emerald-400 border-b border-zinc-800 pb-1.5">
                <span className="flex items-center gap-1.5 text-xs">
                  <Activity className="w-3.5 h-3.5 shrink-0" /> Redis Pub/Sub Stream
                </span>
                <span className="text-[10px] text-zinc-500">SSE Chunk 256KB/s</span>
              </div>
              <div className="text-[10px] sm:text-[11px] text-zinc-400 space-y-1 break-all">
                <p>
                ...
                <p>
                  <span className="text-zinc-600">[14:02:15]</span> <span className="text-cyan-400">STATUS_BROADCAST:</span> p95={currentLatency}ms to connected clients
                </p>
              </div>
            </div>

            <div className="p-2.5 sm:p-3 rounded-lg bg-zinc-900/50 border border-zinc-800 text-xs flex flex-col xs:flex-row xs:items-center justify-between gap-1 text-zinc-400">
              <span className="flex items-center gap-1.5">
                <Shield className="w-3.5 h-3.5 text-indigo-400 shrink-0" />
                API Key Authentication
              </span>
              <span className="font-mono text-emerald-400 text-[10px] sm:text-[11px]">Scoped: `read:telemetry`</span>
            </div>
          </div>
        )}

        {activeTab === "ai-triage" && (
          <div className="p-3 sm:p-3.5 rounded-lg bg-zinc-900/90 border border-zinc-800 space-y-3">
            ...
            <div className="text-xs text-zinc-400 leading-relaxed bg-zinc-950 p-2.5 sm:p-3 rounded border border-zinc-800/80">
              <p className="text-zinc-300 font-mono text-[10px] sm:text-[11px] mb-1">Diagnostics Summary:</p>
